...
def load_files():
    ...
    p0783_component_path = os.path.join(data_path, 'HN_comp_p0783.csv')

    b0783_order_sentence_path = os.path.join(data_path, 'os_detail_b0783.csv')
    b0783_component_path = os.path.join(data_path, 'HN_comp_b0783.csv')
...
if __name__ == '__main__':
    ...
    for powerplan, p0783_powerplan_attr in p0783.get('powerplans').items():
        ...
        if not is_key_val_len_equal(p0783_powerplan_attr, b0783_powerplan_attr, 'phases'):
            print('Mismatch between number of phases in PowerPlan {}'.format(p0783_powerplan))
        ...
        if p0783_phase_names.difference(b0783_phase_names):
            print('Mismatched names within PowerPlan {}'.format(p0783_powerplan))

        for phase in p0783_powerplan_attr.get('phases'):
            p0783_phase_attr = p0783_powerplan_attr \
                                    .get('phases') \
                                    .get(phase)
            p0783_components = sorted(p0783_phase_attr.get('components'), key=lambda i: i.get('sequence'))
            b0783_phase_attr = b0783_powerplan_attr \
                                    .get('phases') \
                                    .get(phase)
            b0783_components = sorted(b0783_phase_attr.get('components'), key=lambda i: i.get('sequence'))

            b0783_synonyms = [x.get('synonym') for x in sorted(b0783_components, key=lambda i: i.get('sequence'))]
            p0783_synonyms = [x.get('synonym') for x in sorted(p0783_components, key=lambda i: i.get('sequence'))]
            combined_synonym_list = lcs.diff_wrapper(b0783_synonyms, p0783_synonyms)
            if b0783_synonyms != p0783_synonyms:
                diff_synonyms = [x for x in combined_synonym_list if '+' in x[0] or '-' in x[0]]
                print('PowerPlan: {}, Phase: {}, LCS: {}'.format(powerplan, phase,  diff_synonyms))
                diff_synonyms_1 = [x for x in combined_synonym_list if '+' in x[0]]
                diff_synonyms_2 = [x for x in combined_synonym_list if '-' in x[0]]
            ...
            for synonym in combined_synonym_list:
                idx_p0783 = 0
                idx_b0783 = 0
                if p0783_components[idx_p0783].get('synonym') == synonym and b0783_components[idx_b0783].get('synonym') == synonym:
                    p0783_order_sentences = p0783_components[idx_p0783].get('order_sentences')
                    b0783_order_sentences = b0783_components[idx_b0783].get('order_sentences')

                    for idx, os in enumerate(p0783_order_sentences):
                        for key, val in os.get('order_sentence_details').items():
                            b0783_val = b0783_order_sentences[idx_b0783].get('order_sentence_details').get(key)
                            if val != b0783_val:
                                append_to_output_csv({
                                    'PowerPlan': powerplan,
                                    'Phase': phase,
